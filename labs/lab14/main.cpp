...
#define EQ(x, y)                                                      \
    do                                                                \
    {                                                                 \
        x == y ? printf("[*] %s %d - PASS - Values are equal!\n",     \
...
#define FOR_EACH_STRING(sl, x, y)                                                       \
    ...
    {                                                                                   \
        ...
        {                                                                               \
            y(sl[i]) == x(sl[i]) ? printf("[*] %s %d - PASS - Values are equal!\n",     \
                                          __FUNCTION__, __LINE__)                       \
                                 : printf("[x] %s %d - FAIL - Values are not equal!\n", \
                                          __FUNCTION__, __LINE__);                      \
        }                                                                               \
...
#define FOR_EACH_STRING2(sl, x, y, ...)                                             \
    ...
    {                                                                               \
        ...
        {                                                                           \
            ...
            c1 == c2 ? printf("[*] %s %d - PASS - Values are equal!\n",             \
                              __FUNCTION__, __LINE__)                               \
...
#define EQ_STRING(s1, s2)                                                                                                                                                                                                  \
    do                                                                                                                                                                                                                     \
    {                                                                                                                                                                                                                      \
        (s1 == s2 && (NULL == s1 || (0 == strcmp(s1, s2)))) ? printf("[*] %s | %d - PASS - Values are equal!\n", __FUNCTION__, __LINE__) : printf("[x] %s | %d - FAIL - Values are not equal!\n", __FUNCTION__, __LINE__); \
    } while (0)
...
#define GET_SEED(s)                                                        \
    ...
    {                                                                      \
        unsigned int a, d;                                                 \
        asm("rdtsc" : "=a"(a), "=d"(d));                                   \
        s = (static_cast<uint64_t>(a) | (static_cast<uint64_t>(d) << 32)); \
    } while (0)

extern "C" size_t ex_strlen(const char *);
...
extern "C" int ex_strcmp(const char *, const char *);
extern "C" int ex_memcmp(void *, void *, size_t);
extern "C" void ex_strcpy(char *, const char *);
extern "C" int ex_atoi(const char *);
...
void mem_tests()
{
    ...
    {
        ex_memcpy(buf2, (void *)stringlist[i], (strlen(stringlist[i]) - 1));
...
void strstr_test()
{
    for (int i = 0; stringlist[i] != NULL; ++i)
    {
        ...
        {
            char *tmp1 = strstr((char *)stringlist[i], needles[j]);
            char *tmp2 = ex_strstr((char *)stringlist[i], (char *)needles[j]);
            EQ_STRING(tmp1, tmp2);
        }
    }
}
